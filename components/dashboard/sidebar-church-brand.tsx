...
export function SidebarChurchBrand({ className }: SidebarChurchBrandProps) {
  ...
  return (
    <Link
      ...
      className={cn(
        "group relative block overflow-hidden rounded-2xl border border-border/60 bg-gradient-to-br from-domain-members-subtle via-card to-attention-subtle p-3.5 shadow-xs transition-[box-shadow,transform,border-color] duration-200 hover:border-domain-members/30 hover:shadow-popover",
    ...
    >
      <div
        className="pointer-events-none absolute -left-8 -top-10 size-28 rounded-full bg-domain-members/18 blur-2xl transition-opacity group-hover:opacity-100"
        aria-hidden
      />
      <div
